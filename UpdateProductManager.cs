...
public class UpdateProductManager : MonoBehaviour
{
    ...
    //int imgChk = 0;
    bool areaChk = true; // 구역을 설정했는지 확인하는 변수
    bool priceChk = true; // 가격을 입력했는지 확인하는 변수
    ...
    public void SettingProduct()
    {
        ...
        if (expHour > 12)
        {
            ...
            GameObject.Find("ExpiryDate/Date/Clock/Time").GetComponent<Text>().text = "오후 " + expHourTxt + ":" + expMinuteTxt;
            expHour += 12;
        }
        else
        {
            ...
            if (expMinute < 10)
                expMinuteTxt = "0" + expMinute;
            GameObject.Find("ExpiryDate/Date/Clock/Time").GetComponent<Text>().text = "오전 " + expHourTxt + ":" + expMinuteTxt;
        }
        if (alarmHour == 12)
            alarmHour = 0;
        ...
        {
            isAlarm = false;
            GameObject.Find("SetAlarm/SetAlarmBtn/Text").GetComponent<Text>().text = "설정하지 않음";
        ...
        else
        {
            ...
            if (alarmHour>12)
            {
                ...
                GameObject.Find("SetAlarm/SetAlarmBtn/Text").GetComponent<Text>().text = (setAlarm) + "일 전 오후 " + alarmHourTxt + ":" + alarmMinuteTxt;
                alarmHour += 12;
            }
            else
            {
                ...
                if (alarmMinute < 10)
                    alarmMinuteTxt = "0" + alarmMinute;
                GameObject.Find("SetAlarm/SetAlarmBtn/Text").GetComponent<Text>().text = (setAlarm) + "일 전 오전 " + alarmHourTxt + ":" + alarmMinuteTxt;
            }
            if (alarmHour == 12)
                alarmHour = 0;
        }
        ...
        else
        {
            ...
            if (checkSave)
            {
                ...
                else
                {
                    ...
                    NotificationManager.SendWithAppIcon(pn, time, "유통이", content, Color.green, NotificationIcon.Clock);

                    content = GameObject.Find("ProductName/ProductNameInput/Text").GetComponent<Text>().text + " 상품의 유통기한이 만료되었습니다.";
                    NotificationManager.SendWithAppIcon(pn + 1000000, expDay - DateTime.Now, "유통이", content, Color.green, NotificationIcon.Clock);
                }
